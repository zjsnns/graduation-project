...
class MultiStepCNNBaseline(nn.Module):
    def __init__(
        self,
        input_dim: int,
        hidden_size: int,
        ...
        self.output_net = nn.Sequential(
            nn.Linear(conv_channels, head_hidden_dim),
            nn.ReLU(),
            nn.Dropout(dropout),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Strict dynamic-cleaning CNN baseline (same split/normalization protocol)."
    )
    ...
    parser.add_argument("--output_root", type=str, default=str(SCRIPT_DIR / "cnn_runs_strict_baseline"))
...
def save_run_config(
    ...
    lines = [
        ...
        f"device={device}",
        f"baseline_name=strict_cnn_baseline",
        ...
        f"nhead={args.nhead}  # unused in CNN, kept for config compatibility",
