    ...
    def forward(
        ...
        if position_ids is None:
            ...
            if position_ids.shape[-1] != seq_length:
                position_ids = position_ids[:, ::self.config.input_token_len] // self.config.input_token_len
            position_ids = position_ids.view(-1, seq_length).long()

        # Resize attention_mask to match the patched sequence length
        if attention_mask is not None and attention_mask.shape[-1] != inputs_embeds.shape[1]:
            attention_mask = attention_mask[:, ::self.config.input_token_len]
